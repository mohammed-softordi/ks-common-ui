...
<head>
  ...
  <script type="module" src="../../iron-iconset/iron-iconset.js"></script>
...
function iconElementFor(node) {
  var nodes = dom(node.root).childNodes;
...
suite('<iron-icon>', function() {
  ...
  suite('lifecycle', function() {
    ...
    teardown(function() {
      if (icon.parentNode != null) {
        dom(icon.parentNode).removeChild(icon);
      }
    });
    ...
    test('does not create icon until attached', function() {
      ...
      var children = dom(icon.root).querySelectorAll('img,svg');

      expect(children.length).to.be.eql(0);
      dom(document.body).appendChild(icon);
      flush();
      children = dom(icon.root).querySelectorAll('img,svg');
  ...
  suite('when switching between src and icon properties', function() {
    ...
    test(
        'will display the icon when src is defined first and then reset',
        function() {
          icon.src = '../demo/location.png';
          icon.icon = null;
          icon.src = null;
    ...
    test(
        'will display the src when icon is defined first and then reset',
        function() {
          icon.src = null;
          icon.icon = 'example:location';
          icon.src = '../demo/location.png';
