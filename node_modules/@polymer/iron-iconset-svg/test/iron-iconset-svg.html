...
suite('<iron-iconset>', function() {
  ...
  suite('when paired with a size and SVG definition', function() {
    ...
    test('can be queried for all available icons', function() {
      expect(iconset.getIconNames())
          .to.deep.eql(['my-icons:circle', 'my-icons:square', 'my-icons:rect']);
    });
    ...
    test('prefers a viewBox attribute over the iconset size', function() {
      ...
      expect(div.firstElementChild.getAttribute('viewBox'))
          .to.be.equal('0 0 50 25');
    });

    test(
        'uses the iconset size when viewBox is not defined on the element',
        function() {
          iconset.applyIcon(div, 'circle');
          expect(div.firstElementChild.getAttribute('viewBox'))
              .to.be.equal('0 0 20 20');
        });
  });
  ...
  suite('Adding / removal from iron-icon', function() {
    ...
    test(
        'be able to remove an iconset from a standard DOM element', function() {
          iconset.applyIcon(div, 'circle');
          flush();
    ...
    test('be able to remove an iconset from a Polymer element', function() {
      ...
      flush();
      expect(dom(ironIcon.root).children.length - baseLength)
          .to.be.equal(1);
    ...
    test('prevents SVG from receiving a tab stop in IE/Edge', function() {
      iconset.applyIcon(div, 'rect');
      expect(div.firstElementChild.getAttribute('focusable'))
          .to.be.equal('false');
    });
  });
});
