...
<!--
`c-highcharts` Spline charts integrate all the characteristics of a conventional line chart except for the fact that unlike line charts, which connect data points with straight lines, spline charts draw a fitted curved line to join the data points.
...
<dom-module id="c-highcharts">
  ...
  <script src="/vendors/components/pure-js/highcharts/5.0.14/highstock.js"></script>
  <script src="/vendors/components/pure-js/highcharts/5.0.14/modules/exporting.js"></script>
  <script src="/vendors/components/pure-js/highcharts/5.0.14/modules/drilldown.js"></script>
  <link rel="import" href="variation-1.html"/>
  <template>
    <style>:host {
  width: 100%;
  height: var(--highcharts-height);
}
:host #Chart {
  height: inherit;
}
:host #Chart .highcharts-container {
...
<content></content>
  ...
  <script>var name = 'c-highcharts'; Highcharts.setOptions({
...
Highcharts.Polymer_BaseBehavior = {
    ...
    setData: function (x, z) {
        ...
        if (!isArr) {
            x = $.extend({ name: (this.label || this.yLabel || this.xLabel), colorByPoint: this.colorByPoint }, x);
        }
        if (x && x.length && x[0].data instanceof Array) {
            __app.async(function () { while (__app._chart.series.length) {
                __app.removeSeries(0, false);
            } ; x.forEach(function (d) { __app._chart.addSeries(d); }); }, __app.__microTaskDelaySetData);
        }
        else {
            __app._getSeries(z)[isArr ? "setData" : "update"](x);
        }
    },
    ...
    updateSeries: function (k, v, z) { var NewEl = {}; if (typeof k == "object") {
        NewEl = k;
        z = v || z;
    }
    else {
        NewEl[k] = v;
    } ; this._getSeries(z).update(NewEl); },
    resizeChart: function () { if (!this._chart) {
        return;
    } this._chart.reflow(); },
    resizeChartFixed: function () { if (!this._chart) {
        return;
    } this._chart.setSize(this.$.Chart.offsetWidth, this.offsetHeight); },
    reRender: function () { if (!this._chart) {
        return;
    } this._set_chart(new Highcharts.Chart(this._chart.options)); this.setData(this.data); this.__lateBinders(); },
    downloadAs: function (name, otherOptions) { if (!this._chart) {
        return;
    } this._chart.exportChart($.extend({ filename: name }, otherOptions)); },
    destroy: function () { if (!this._chart) {
        return;
    } this._chart.destroy(); },
    showLoading: function (d) { this.loadingMessage = d; this.loading = true; },
    zoomOut: function () { if (!this._chart) {
        return;
    } ; this._chart.zoomOut(); },
    getSeries: function (z) { return this._getSeries(z); },
    _getSeries: function (z) { if (!this._chart) {
        return this._returnDummySeries();
    } ; z = typeof z != "number" ? 0 : z; var s = this._chart.series; if (!s.length) {
        this._warn("Chart is empty [no series]");
        return this._returnDummySeries();
    } ; if (z < s.length) {
        return s[(z || 0)];
    }
    else {
        this._warn("Index z out of bounds");
        return this._returnDummySeries();
    } },
    ...
    _creditsUpdate: function (c) { if (!this._chart) {
        return;
    } ; this._chart.update({ credits: { enabled: c } }); },
    _legendUpdate: function (s) { if (!this._chart) {
        return;
    } ; this._chart.legend.update({ enabled: s }); },
    _legendOptionsUpdate: function (o) { if (!this._chart) {
        return;
    } ; this._chart.legend.update(o); },
    _tooltipOptionsUpdate: function (o) { if (!this._chart) {
        return;
    } ; this._chart.tooltip.update(o); },
    _plotUpdate: function (o) { if (!this._chart) {
        return;
    } ; this._chart.update({ plotOptions: o }); },
    _chartUpdate: function (o) { if (!this._chart) {
        return;
    } ; Object.keys(o || {}).length && this._warn("Not doing what you wanted, maybe you meant plotOptions?", "chartOptions"); this._chart.update({ chart: o }); },
    _exportingUpdate: function (x) { if (!this._chart) {
        return;
    } ; this._chart.update({ exporting: { enabled: x } }); },
    _updateType: function () { if (!this.type) {
        return;
    } ; this.updateSeries("type", this.type); },
    _cbpUpdate: function () { this.updateSeries("colorByPoint", this.colorByPoint); },
    _xAxisUpdate: function () { if (!this._chart) {
        return;
    } ; this._chart.xAxis[0].update(this.xAxis); },
    _yAxisUpdate: function () { if (!this._chart) {
        return;
    } ; this._chart.yAxis[0].update(this.yAxis); },
    _zoomUpdate: function () { if (!this._chart) {
        return;
    } ; this._chart.update({ chart: { zoomType: "" + (this.xZoom ? "x" : '') + (this.yZoom ? "y" : '') } }); },
    _xAxisTitleUpdate: function () { if (!this._chart) {
        return;
    } ; this._chart.xAxis[0].update({ title: { text: this._getAxisLabel('X') } }); },
    _yAxisTitleUpdate: function () { if (!this._chart) {
        return;
    } ; this._chart.yAxis[0].update({ title: { text: this._getAxisLabel('Y') } }); },
    _labelUpdate: function () { if (!this._chart) {
        return;
    } ; this._xAxisTitleUpdate(); this._yAxisTitleUpdate(); },
    _titleUpdate: function () { if (!this._chart) {
        return;
    } ; this._chart.setTitle({ text: this.title }); },
    _loadingUpdate: function () { if (!this._chart) {
        return;
    } ; this._chart[(this.loading ? "show" : "hide") + "Loading"](this.loadingMessage); },
    _subtitleUpdate: function () { if (!this._chart) {
        return;
    } ; this._chart.setTitle(null, { text: this.subtitle }); },
    _hcUpdate: function (o) { if (!this._chart) {
        return;
    } ; this._chart.update(o); },
    //Other Bindings
    _getAxisLabel: function (dim) { var l = dim.toLowerCase(), u = dim.toUpperCase(), lab = this[l + "Label"] || this.label; if (lab == u + "-Axis") {
        lab = this.label || lab;
    } ; return lab.trim(); },
    ...
    __addLateBinder: function (cb) { if (!(cb instanceof Array)) {
        cb = [cb];
    } ; if (!this.__lateBindCallbacks) {
        this.__lateBindCallbacks = {};
    } ; var me = this; cb.forEach(function (c) { me.__lateBindCallbacks[c] = 1; }); }
};
Highcharts.Polymer_ChartBehavior = {
    ...
    _vsTimeUpdate: function (a) { if (!this._chart || typeof a == "undefined") {
        return;
    } ; this.destroy(); this.ready(); },
    _checkSelected: function () { if (!this._chart) {
        return;
    } ; var points = this._chart.getSelectedPoints(); this._setSelected(!!points.length); this._setSelectedPoints(points); }
};
Polymer({
    is: name,
    behaviors: [
