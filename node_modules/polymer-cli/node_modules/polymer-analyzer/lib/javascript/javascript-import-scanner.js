...
class ScannedJavascriptImport extends model_1.ScannedImport {
    constructor(url, sourceRange, urlSourceRange, ast, lazy, originalSpecifier, statementAst) {
    ...
    constructImport(resolvedUrl, relativeUrl, importedDocument, _containingDocument) {
        return new JavascriptImport(resolvedUrl, relativeUrl, this.type, importedDocument, this.sourceRange, this.urlSourceRange, this.astNode, this.warnings, this.lazy, this.specifier, this.statementAst);
    }
}
exports.ScannedJavascriptImport = ScannedJavascriptImport;
class JavascriptImport extends model_1.Import {
    constructor(url, originalUrl, type, document, sourceRange, urlSourceRange, ast, warnings, lazy, specifier, statementAst) {
        ...
        return __awaiter(this, void 0, void 0, function* () {
            ...
            yield visit({
                enterCallExpression(node, _, path) {
                    ...
                    imports.push(new ScannedJavascriptImport(scanner._resolveSpecifier(specifier, document, node, warnings), document.sourceRangeForNode(node), document.sourceRangeForNode(node.callee), { language: 'js', node, containingDocument: document }, true, specifier, esutil.getCanonicalStatement(path)));
                },
                enterImportDeclaration(node, _, path) {
                    const specifier = node.source.value;
                    imports.push(new ScannedJavascriptImport(scanner._resolveSpecifier(specifier, document, node, warnings), document.sourceRangeForNode(node), document.sourceRangeForNode(node.source), { language: 'js', node, containingDocument: document }, false, specifier, esutil.getCanonicalStatement(path)));
                },
